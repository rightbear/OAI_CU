...
void apply_pdcp_config(rrc_gNB_ue_context_t         *const ue_context_pP,
      ...
      printf("\n[MWNL_TEST] FUNCTION: %s, LINE: %d", __FUNCTION__, __LINE__);
...
//-----------------------------------------------------------------------------
{
  ...
  if (CN_ENABLED) { // AMF_MODE_ENABLED
...
{
  printf("[MWNL TEST] FUNCTION: %s, LINE: %d\n", __FUNCTION__, __LINE__);
  int                                 drb_id;
  ...
  if (ul_dcch_msg->message.present == NR_UL_DCCH_MessageType_PR_c1) {
    switch (ul_dcch_msg->message.choice.c1->present) {
      ...
      case NR_UL_DCCH_MessageType__c1_PR_rrcSetupComplete:
        ...
        if(!CN_ENABLED)
        {
          if (ue_context_p->ue_context.established_pdu_sessions_flag == 1) { //ue_context_p->ue_context.established_pdu_sessions_flag == 1
...
void rrc_gNB_process_f1_setup_req(f1ap_setup_req_t *f1_setup_req) {
  ...
  F1AP_SETUP_RESP (msg_p).num_cells_to_activate = 0; // OAI :0 , ORAN need to send F1setupRsp which carrys cell active list
  ...
  for (int i = 0; i < f1_setup_req->num_cells_available; i++) {
	...
	F1AP_SETUP_RESP (msg_p).gNB_CU_name  = rrc->node_name;
        ...
        /* OAI original
        asn_dec_rval_t dec_rval = uper_decode_complete(NULL,  // uper
                                  &asn_DEF_NR_BCCH_BCH_Message,
                                  (void **)&rrc->carrier.mib_DU,
                                  f1_setup_req->mib[i],
                                  f1_setup_req->mib_length[i]);
        */
      for(int k=0; k<f1_setup_req->mib_length[i]; k++)
      {
          printf("%x ", f1_setup_req->mib[i][k]);
      }
      printf("\n");
      //ORAN decode
       asn_dec_rval_t dec_rval = aper_decode(NULL,
        ...
        AssertFatal(dec_rval.code == RC_OK,
                    ...
                    j,
                    dec_rval.consumed );
        xer_fprint(stdout, &asn_DEF_NR_MIB, rrc->carrier.Mib);
        
        NR_BCCH_BCH_Message_t *mib = &rrc->carrier.mib;
        //NR_BCCH_BCH_Message_t *mib_DU = rrc->carrier.mib_DU; ORAN disable 
        ...
        memcpy(mib->message.choice.mib, rrc->carrier.Mib, sizeof(struct NR_MIB)); // ORAN change from mib_DU->message.choice.mib to rrc->carrier.Mib
        ...
        for(int k=0; k<f1_setup_req->sib1_length[i]; k++)
        {
            printf("%x\t", f1_setup_req->sib1[i][k]);
        }
        printf("\n");
        ...
        */
        ...
        dec_rval = aper_decode_complete(NULL,
                    &asn_DEF_NR_SIB1,   //&asn_DEF_NR_BCCH_DL_SCH_Message &asn_DEF_NR_SIB1
                    (void **)&rrc->carrier.sib1,//siblock1_DU sib1
                    f1_setup_req->sib1[i],
        ...
        AssertFatal(dec_rval.code == RC_OK,
                    ...
                    j,
                    dec_rval.consumed );
        xer_fprint(stdout, &asn_DEF_NR_SIB1, rrc->carrier.sib1);
        // Parse message and extract SystemInformationBlockType1 field
        NR_SIB1_t *Sib1 = rrc->carrier.sib1;
        /* OAI original
  ...
  /* ORAN do not send F1_gNB_CU_CONFIGURATION_UPDATE
  ...
  */
  ...
  F1AP_SETUP_RESP (msg_p).num_cells_to_activate = cu_cell_ind;
    ...
    if (cu_cell_ind == 0) {
      ...
      //ORAN do not send F1_gNB_CU_CONFIGURATION_UPDATE
      //itti_send_msg_to_task (TASK_CU_F1, 0, msg_p2); // ORAN DU cannot handle GNB_CU_CONFIGURATION
