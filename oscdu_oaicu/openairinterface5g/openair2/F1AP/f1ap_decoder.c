...
static int f1ap_decode_initiating_message(F1AP_F1AP_PDU_t *pdu) {
  ...
  switch(pdu->choice.initiatingMessage->procedureCode) {
    ...
    /* O-RAN send F1_gNB_DU_ConfigurationUpdate, OAI CU needs to cover */
    case F1AP_ProcedureCode_id_gNBDUConfigurationUpdate:
      LOG_I(F1AP, "%s(): F1AP_ProcedureCode_id_gNBDUConfigurationUpdate\n", __func__);
      break;
