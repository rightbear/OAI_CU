...
int CU_handle_INITIAL_UL_RRC_MESSAGE_TRANSFER(instance_t             instance,
  ...
  uint8_t ueId; // O-RAN set gNB DU/CU UE F1AP ID as ueId
  ...
  ueId = ie->value.choice.GNB_DU_UE_F1AP_ID; // ORAN added
  /* NRCGI
  * Fixme: process NRCGI
  */
  F1AP_FIND_PROTOCOLIE_BY_ID(F1AP_InitialULRRCMessageTransferIEs_t, ie, container,
  ...
  int f1ap_uid = f1ap_add_ue(CUtype, instance, rnti, ueId); // O-RAN: Let gNB_CU/DU_UE_F1AP_ID as UEID 
