  ...
  mem_block_t *const sdu_buffer)
{
  ...
  if (srb_flagP == 1) {
    ...
    printf("[MWNL_TEST] pdcp rb : %p\n", rb);
  if (rb != NULL) {
    
    printf("[MWNL TEST] FUNCTION: %s ,LINE: %d\n", __FUNCTION__, __LINE__);
    printf("[MWNL TEST] pdcp sdu buffer size: %d\n", sdu_buffer_size);
    rb->recv_pdu(rb, (char *)sdu_buffer->data, sdu_buffer_size); // MWNL TEST Check why security mode complete received rb is NULL
  } else {
